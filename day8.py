...
lines = [ [ c.split() for c in x.strip().split('|') ] for x in open('test8.txt') ]
...
for l in lines:
    ...
    mappings[be.union(ten[invdm[4][0]]).pop()] = set('b')
    ...
    print([ lookup[''.join(sorted(list(mappings[s])[0] for s in v))] for v in l[1]])


        #easy = { for i,d in dm.items():
    ...
    #         for s in l[0][i]:
    #             for d in dm[i]:
    #                 mappings[s].difference_update(idx2[d])
    #         print(i, l[0][i], dm[i])
    #         for x in sorted(mappings):
    #             print(x, ':', "".join(sorted(mappings[x])))

    #     if all(len(m)==1 for m in mappings.values()): break
    #     break
